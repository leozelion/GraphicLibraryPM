...
namespace Graphic
{
    ...
    class ShellSphereModelNew : ShellBaseModel
    {
       ...
       //конструктор
        public ShellSphereModelNew(Device Device, float Radius, float StartXAngle, float SizeXAngle, float SizeYAngle, float SizeZ, List<Vector3> Progibs, float Offset, double scaleCoeff1, string prgfun1)
        {
            device = Device;
            radius = Radius;
        ...
        //отрисовка
        public override void Draw(Device device, Matrix view_matrix)
        {
            if (autoAnimation)
            {
        ...
        private void BuildModel()
        {
            ...
            vertexesMiddle = HelpClassNew.BuildSphere(radius, setkaX, setkaY, 0);
    ...
    //сферическая оболочка с ребрами жесткости
    public class ShellSphereModellWithEdgesNew : ShellBaseModel
    {
        float radius; //радиус
        float startXAngle; //угловая стартовая координата по X
        float sizeXAngle; //угловой размер по оси X (длина, ширина) в радианах
        ...
        public float AnimationWeight
        {
            ...
            set
            {
                ...
                foreach (ShellSphereModelNew item in rebroX)
                    item.AnimationWeight = value;
                foreach (ShellSphereModelNew item in rebroY)
                    item.AnimationWeight = value;
        ...
        List<ShellSphereModelNew> rebroX = new List<ShellSphereModelNew>();
        List<ShellSphereModelNew> rebroY = new List<ShellSphereModelNew>();
        ShellSphereModelNew main;

        public ShellSphereModellWithEdgesNew()
        {
        }

        public bool CreateModel(Device Device, float Radius, float StartXAngle, float SizeXAngle, float SizeYAngle, float SizeZ, int NHX, int NHY, float HWX, float HWY, float SizeRebroXAngle, float SizeRebroYAngle, ProgibFunctionDelegate ProgibFunction, int dummy, float scaleCoeff, string prgfun)
        {
            try
            {
                ...
                return CreateModel(Device, Radius, StartXAngle, SizeXAngle, SizeYAngle, SizeZ, NHX, NHY, HWX, HWY, SizeRebroXAngle, SizeRebroYAngle, Progibs, scaleCoeff, prgfun);
            }
            catch
            {
                return false;
        ...
        public bool CreateModel(Device Device, float Radius, float StartXAngle, float SizeXAngle, float SizeYAngle, float SizeZ, int NHX, int NHY, float HWX, float HWY, float SizeRebroXAngle, float SizeRebroYAngle, List<Vector3> Progibs, float scaleCoeff1, string prgfun1)
        {
            try
            {
                device = Device;
                ...
                if (nhx > 0)
                {
                    ...
                    for (int i = 0; i < nhx; i++)
                    {
                        ...
                        rebroX.Add(new ShellSphereModelNew(device, radius, startXAngle, sizeXAngle, sizeYAngle, hwx, progibsRebro, -sizeZ / 2 - hwx / 2, scaleCoeff1, prgfun1));
                    }
                }
                if (nhy > 0)
                {
                    ...
                    for (int i = 0; i < nhy; i++)
                    {
                        ...
                        rebroY.Add(new ShellSphereModelNew(device, radius, startXAngle, sizeXAngle, sizeYAngle, hwy, progibsRebro, -sizeZ / 2 - hwy / 2, scaleCoeff1, prgfun1));
                    }
                }

                main = new ShellSphereModelNew(device, radius, startXAngle, sizeXAngle, sizeYAngle, sizeZ, progibs, 0, scaleCoeff1, prgfun1);
            }
            catch
            {
                return false;
        ...
        public override void Draw(Device device, Matrix matrix)
        {
            ...
            if (rebroX.Count > 0)
                foreach (ShellSphereModelNew item in rebroX)
                    item.Draw(device, matrix);
            if (rebroY.Count > 0)
                foreach (ShellSphereModelNew item in rebroY)
                    item.Draw(device, matrix);

            main.Draw(device, matrix);
